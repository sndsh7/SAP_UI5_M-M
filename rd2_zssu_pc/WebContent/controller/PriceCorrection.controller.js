sap.ui.define(["sap/ui/core/mvc/Controller",
  ...
  var s2;
  var prevAssoName;
  return Controller.extend("com.mahindra.ZSSU_PriceCrctn.controller.PriceCorrection", {
    onInit: function () {
      // Fiori Usage
      var device = "Other";
      var os = jQuery.os.os;
      ...
      oDataModel.read("/FioriUsageSet(Appname='ZSSU_PriceCrctn',Device='" + device +
...
//      oController._onObjectMatched(["new","PC"]);
    },
    ...
    _onObjectMatched: function (oParameter) {
      ...
      oController.getView().byId("redelegation").setVisible(false) // added by AGAWSA-CONT SF1K92101
      var sUrl = jQuery.sap.getModulePath('com.mahindra.ZSSU_PriceCrctn', '/json/PriceCrctn.json');
      ...
      if (vPaperNo == "new") {
        ...
        oModel.setProperty("/SSURequest", false);	// Added by AGAWSA-CONT SF1K92101
        var oData = getHeaderObjectPC();
//        oData.Nav_Currency.push(getCurrencyStr());
        oData.PaperCode= vPaperCode;
        oModel.setProperty("/PaperDataSet", oData);
//        oController._getAmentmendService(); // Remove by AGAWSA-CONT on 04.09.2023
        oController._OgAmentmendCode(vPaperCode);
...
//    Insert start by AGAWSA-CONT on 04.09.2023
      oController.getOwnerComponent().getModel("oDataNewModel").read("/ES_PaperList", {
      urlParameters: {
...
//        debugger;
        const vData = oData.results.filter(function(papercode){
              return papercode.PaperCode != 'PPP' &&
                   papercode.PaperCode != "AS" &&
                   papercode.PaperCode != "PC" &&
                   papercode.PaperCode != "SPD"; // AS,SPD
            })
        oModel.setProperty("/CorrectionCat", vData);
        oModel.refresh();
...
//    Insert end by AGAWSA-CONT on 04.09.2023
...
//    Added by AGAWSA-CONT- 05-06-2023
...
//      END ADD by AGAWSA-CONT
    ...
    _fnAfterGettingBEData: function (oData) {
      ...
      com.mahindra.ZSSU_PriceCrctn.oData = oData;	// Added by AGAWSA-CONT SF1K92101
       
      var vCurrentUser = sap.ushell.Container.getService("UserInfo").getId(); // Added by AGAWSA-CONT SF1K92101
...
//      oData.Sign = oData.Sign == "POS" ? 0 : 1; //0 is Increase 1 is Decrease;
      $.each(oData.Nav_Items, function (i, Row) {
        ...
        if(typeof Row.LastAmmndDate=="string"){
           Row.LastAmmndDate = new Date(Row.LastAmmndDate);
        }
      });
...
//      oController.getView().byId("id_export").setVisible(true);
      ...
      this.getView().byId("id_pc").setEditable(false); // Add by AGAWSA-CONT on 15.09.2023
//    ADD Start by AGAWSA-CONT SF1K92101
//      oModelSSUDigitization.setProperty("/PaperDataSet", oData);
//      if (oData.Status === "S" || oData.Status === "R")
//        oModelSSUDigitization.setProperty("/DisplayOnlyFlag", false);
...
//      // oController._validationDataFilled();
    ...
    _fnDataBeforeSave: function () {
      ...
      $.each(oPaperData.Nav_Items, function (i, Row) {
        ...
        Row.LastAmmndDate = uiDateToBackend(Row.LastAmmndDate);
      });
      return oPaperData;
    },
      ...
      var oActionItem = new sap.m.HBox({
        ...
        items: [
          ...
          new sap.m.Button({
            ...
            press: function (oEvent) {
//              Add Start by AGAWAS-CONT on 05-06-2023
...
//                Add End by AGAWAS-CONT on 05-06-2023
              var oItemFrag = xmlFragment(oController, "ItemPrcCrctn");
...
//                Add End by AGAWAS-CONT on 05-06-2023
          ...
          new sap.m.Button({
            ...
            press: function (oEvent) {
                //added by beena for original code
                var oModelSSUDigitization = oController.getView().getModel("ModelSSUDigitization");
                var date = new Date();
                oModelSSUDigitization.setProperty("/dateForOrigCode", date);
...
//          var vLastAmmndDate = row.Last_Amendment_Date || "";
//
//          var oItemData = getItemDetailsObjectPC();
...
//          oItemData.LastAmmndDate = new Date(vLastAmmndDate.split(".").reverse()) != "Invalid Date" ? new Date(vLastAmmndDate.split(".").reverse()) : null;
...
//        var vLastAmmndDate = row.Last_Amendment_Date || "";
        var vLastAmmndDate = row.Last_AmendDate || ""; // Added AGAWSA-CONT on 07.09.2023
        vLastAmmndDate = new Date(vLastAmmndDate.split(".").reverse()) != "Invalid Date" ? new Date(vLastAmmndDate.split(".").reverse()) : null;
        var vPlant = row.Plant || "";
        var vOgAmndcode = row.OgAmend_Code || "";
        var vVendor = row.Vendor_Code || "";
        var vPartNo = row.Part_Number || "";
        var vPinforcat = row.Doc_Type || "";
        var vSettledPrice = row.Settled_Price || "0";
        var vAmend_Code = row.Amend_Code || ""; // Added AGAWSA-CONT on 07.09.2023
        ...
        if (oFilter.length <= 0) {
          ...
          oItemData.OgAmndcode = vOgAmndcode;
          oItemData.LastAmmndDate = vLastAmmndDate;
          oItemData.Amndcode = vAmend_Code; // Added AGAWSA-CONT on 07.09.2023
          ...
          oMassData[existingIndex].LastAmmndDate = vLastAmmndDate;
          oMassData[existingIndex].Amndcode = vAmend_Code; // Added AGAWSA-CONT on 07.09.2023
          oMassData[existingIndex].OgAmndcode = vOgAmndcode;
...
//        }
    ...
    _validationMass: function (Nav_Items) {
      ...
      var oValidation = {
        ...
        "CorrectionCat": oModelSSUDigitization.getProperty("/PaperDataSet/CorrectionCat"),
        "Nav_ItemsX": []
      };
      $.each(Nav_Items, function (item, Row) {
        ...
        var oNavItem = {
          ...
          "OgAmndcode" : Row.OgAmndcode,
          "LastAmmndDate" : uiDateToBackend(Row.LastAmmndDate)
        };
...
//          var vTitle = "Information";
          ...
          $.each(oData.Nav_ItemsX, function (i, Row) {
            ...
            if (Row.VendorFlag == "") {
              ...
              Nav_Items[vIndexItem].LastAmmndDate = Row.LastAmmndDate;
              ...
              Nav_Items[vIndexItem].OgAmndcode = Row.OgAmndcode;
              Nav_Items[vIndexItem].OgAmndcodeDesc = Row.OgAmndcodeDesc;
              Nav_Items[vIndexItem].Quantity = Row.Quantity;
              ...
              // Calculating Apprx Cash Flow in Rs
              var vAprxCahFlow = Row.Quantity * vSign;
              Nav_Items[vIndexItem].AprxCashFlow = vAprxCahFlow.toFixed(2);
//            Remove start by AGAWSA-CONT on 14.09.2023
//              if(vSign == 0)
//                vSign = "";
//              else
//                vSign = vSign > 0? "+" : "-";
...
//              }
//              Remove end by AGAWSA-CONT on 14.09.2023
...
//              Nav_Items[vIndexItem].AmndcodeDesc = Row.AmndcodeDesc;
//            }
            if (Row.VendorFlag != "" || Row.PartNoFlag != "" || Row.OgAmndcodeFlag != "" || Row.LastAmmndDateFlag != "" || Row.SettledPriceFlag != "" || Row.AmndcodeFlag != "" || Row.PlantFlag != "" || Row.ValidFromFlag != ""
              || Row.Messg != "") {
...
//        Comment start by AGAWSA-CONT SF1K92101
...
//          });
//          busyDialog.close();
//        Comment end by AGAWSA-CONT SF1K92101
        },
        error: function (oResponse) {
           serviceError(oResponse);
          busyDialog.close();
...
//        if (Row.Plant == "" || Row.Vendor == "" || Row.PartNo == "" || Math.abs(Row.SettledPrice) == "" || Math.abs(Row.SettledPrice) == "0"
//          || Row.Amndcode == "" || Row.ValidFrom == null || Row.LastAmmndDate == null || Row.OgAmndcode == "" ||  Row.Pinforcat == "" || Row.Purorg == "" ||
...
//        }
        if (Row.Plant == "" || Row.Vendor == "" || Row.PartNo == "" || Math.abs(Row.SettledPrice) == "" || Math.abs(Row.SettledPrice) == "0"
            || Row.Amndcode == "" || Row.ValidFrom == null ||  Row.Pinforcat == "" || Row.Purorg == "" ||
            (Row.uiFields.Error == "Reject" && Row.uiFields.MassValidation.hasOwnProperty("ItemNo"))) {
            vFlag = false;
            oTabIconColor.VendorPartDetails = "Negative";
            Row.uiFields.Error = "Reject";
    ...
    onExport: function () {
      ...
      $.each(oNav_Items,function(i,row){
        ...
        vDateLast = uiDateToBackend(vCopyRow.LastAmmndDate).split("T")[0].split("-").reverse();
        vCopyRow.ValidFrom = vDate.join(".");
        vCopyRow.LastAmmndDate = vDateLast.join(".");
        oExcelData.unshift(vCopyRow);
      });
      var oSettings = {
        workbook: {
          columns: getExportExcelColumsPC(),
...
//        new sap.ui.model.Filter("CorrectionCat", sap.ui.model.FilterOperator.EQ, oPaperData.CorrectionCat),
...
//      Insert end by AGAWSA-CONT on 04.09.2023
      ...
      oDataAmendmentModel.read("/ES_F4Amandment", {
        ...
        success: function (oData, oResponse) {
//      Insert start by AGAWSA-CONT on 04.09.2023
          const ids = oData.results.map(({ Zpricecd }) => Zpricecd);
          const filtered = oData.results.filter(({ Zpricecd }, index) =>
              !ids.includes(Zpricecd, index + 1));
//          Insert end by AGAWSA-CONT on 04.09.2023
          oModelSSUDigitization.setProperty("/F4Amentment", filtered);
          oModelSSUDigitization.refresh();
        },
        error: function (oResponse) {
      ...
      _getAmentmendService2: function(vSign){
          var oController = this;
          var oModelSSUDigitization = oController.getView().getModel("ModelSSUDigitization");
          var oPaperData = oModelSSUDigitization.getProperty("/PaperDataSet");
          var oDataAmendmentModel = oController.getOwnerComponent().getModel("oDataAmendmentModel");
...
//          Insert end by AGAWSA-CONT on 04.09.2023
         ...
         _OgAmentmendCode: function(vPaperCode){
      var oController = this;
      var oModelSSUDigitization = oController.getView().getModel("ModelSSUDigitization");
      var oPaperData = oModelSSUDigitization.getProperty("/PaperDataSet");
      var oDataAmendmentModel = oController.getOwnerComponent().getModel("oDataAmendmentModel");
...
//        new sap.ui.model.Filter("Sign", sap.ui.model.FilterOperator.EQ, oPaperData.Sign),
      ...
      oDataAmendmentModel.read("/ES_F4OgAmandment", {
        filters: filters,
        success: function (oData, oResponse) {
          oModelSSUDigitization.setProperty("/F4OgAmentment", oData.results);
          oModelSSUDigitization.refresh();
        },
        error: function (oResponse) {
          Message.error("Error loading Amendment.", {
...
//          ADD End by AGAWSA-CONT on 14-02-2023 validation error message for inactive users
          ...
          oModelSSUDigitization.refresh();
          var vCycle = (oData.results.length>0 && oData.results[0].Cycle) || oModelSSUDigitization.getProperty("/PaperDataSet/Cycle");
...
//         filters.push(new sap.ui.model.Filter("ImportFlag", sap.ui.model.FilterOperator.EQ, vImportFlag));
        filters.push(new sap.ui.model.Filter("OgAmndcode", sap.ui.model.FilterOperator.EQ, oParentModel.getProperty("/OgAmndcode")));
        filters.push(new sap.ui.model.Filter("OgAmndcodeDesc", sap.ui.model.FilterOperator.EQ, oParentModel.getProperty("/OgAmndcodeDesc")));
        filters.push(new sap.ui.model.Filter("LastAmmndDate", sap.ui.model.FilterOperator.EQ, oParentModel.getProperty("/LastAmmndDate")));
        ...
        oDataAmendmentModel.read("/ES_Vendor", {
          ...
          success: function (oData, oResponse) {
            ...
            if(vPinforcat == oData.results[0].Pinforcat || vPinforcat == ""){
              ...
              vOgAmndcode = oData.results[0].OgAmndcode;
              vOgAmndcodeDesc = oData.results[0].OgAmndcodeDesc;
              vLastAmmndDate = oData.results[0].LastAmmndDate;
            }
              else
              {
              ...
              vOgAmndcode = oData.results[1].OgAmndcode;
              vOgAmndcodeDesc = oData.results[1].OgAmndcodeDesc;
              vLastAmmndDate = oData.results[1].LastAmmndDate;
...
//              sap.m.MessageToast.show(vSelectedValue + " either does not exist or not extended for " + oParentModel.getProperty("/Plant") + " plant.");
            ...
            oParentModel.setProperty("/OgAmndcode", vOgAmndcode);
            oParentModel.setProperty("/OgAmndcodeDesc", vOgAmndcodeDesc);
            oParentModel.setProperty("/LastAmmndDate", vLastAmmndDate);
...
//    Quantity service on Change Vaid from date
    onChangeValidFrom : function(oEvent){
      ...
      vFilters.push(new sap.ui.model.Filter("PartNo", sap.ui.model.FilterOperator.EQ, oParentModel.getProperty("/PartNo")));
      vFilters.push(new sap.ui.model.Filter("Vendor", sap.ui.model.FilterOperator.EQ, oParentModel.getProperty("/Vendor")));
      vFilters.push(new sap.ui.model.Filter("ValidFrom", sap.ui.model.FilterOperator.EQ,uiDateToBackend(oParentModel.getProperty("/ValidFrom"))));
      busyDialog.open();
      oDataAmendmentModel.read("/ES_CalcCumRecQty", {
        filters: vFilters,
        success: function (oData, oResponse) {
            var vQuantity = oData.results[0].Quantity;
            oParentModel.setProperty("/Quantity", vQuantity);
            busyDialog.close();

        },
        error: function (oResponse) {
          busyDialog.close();
          Message.error("Error loading Quantity.", {
            title: "Error"
          });
        }
      });
...
//      Insert start by AGAWSA-CONT on 09.10.2023
        formattVisible: function(vData){
          debugger
          if(vData || vData === undefined || vData === ""){
            return false
          }else{
            return true
          }
//          if(vData != "" && vData != null && vData != "000"){
//            return true;
//          }else{
//            return false;
//          }
...
//        Insert start by AGAWSA-CONT on 15.09.2023
        visibleCrrCat: function(PaperNo,CorrectionCat){
          if(CorrectionCat == "" && PaperNo != ""){
            return false;
          }else{
            return true;
          }
...
//      Insert end by AGAWSA-CONT on 15.09.2023
...
////    Date formatter
//    uiDateToBackend: function(oUiDate){
//      if(oUiDate){
...
//      }
//    },
//  Added by AGAWSA-CONT SF1K92101
...
//  Added End by AGAWSA-CONT SF1K92101
//    visiblePartEdit: function(vEditable, vSector, vModel,vInitiatedBySSU) {
//      if (vEditable && vSector && vSector.length > 0 && vModel && vModel.length > 0 && vInitiatedBySSU) {
//        return true;
//      } else {
//        return false;
//      }
//    },
    ...
    formatterSettlePriceEdit : function(vEditable, vValidFrom){
      return vEditable && (vValidFrom != null ? true : false);
    },
...
//    Insert start by AGWSA-CONT on 04.09.2023
    onCorrectionChange: function (oEvent){
      var oController = this;
      var oModelSSUDigitization = oController.getView().getModel("ModelSSUDigitization");
      var vSelCorrectionCat = oEvent.getParameters().selectedItem.mProperties.key;
      oModelSSUDigitization.setProperty("/PaperDataSet/CorrectionCat", vSelCorrectionCat);
      oController._getAmentmendService();

    },
//    Insert end by AGWSA-CONT on 04.09.2023
    ...
    onAddForexItem: function () {
      var oController = this;
//    Insert start by AGWSA-CONT on 10.10.2023
      var oModelSSUDigitization = oController.getView().getModel("ModelSSUDigitization");
//    Insert end by AGWSA-CONT on 10.10.2023
      var oItemFrag = xmlFragment(oController, "ItemPrcCrctn");
      var oModel = new JSONModel(getItemDetailsObjectPC());
//    Insert start by AGWSA-CONT on 10.10.2023
      if(oModelSSUDigitization.oData.PaperDataSet.CorrectionCat == "PYA"){
        var newMinDate = oModel.oData.uiFields.minDate;
        if(newMinDate.getMonth()<3){
          oModel.oData.uiFields.minDate = new Date(newMinDate.getFullYear()-1,3,1);
        }else{
          oModel.oData.uiFields.minDate = new Date(newMinDate.getFullYear(),3,1);
        }
      }
//    Insert end by AGWSA-CONT on 10.10.2023
    ...
    onOkItemDetail: function (oEvent) {
      ...
      var vItemSign = oEvent.getSource().getParent().getModel().getProperty("/Sign");
//      Comment start By AGAWSA-CONT on 07.09.02023
//      if(vItemSign == ""){
//        sap.m.MessageToast.show("Settled Price should not be equals to Existing Price.");
//        return;
//      }
//    Comment end By AGAWSA-CONT on 07.09.02023
...
//                    title: vMsgReturn.Title,
                    onClose: function () {
                      ...
                      oModelSSUDigitization.setProperty("/EditableFlag", false);
                      if(oModelSSUDigitization.oData.PaperDataSet.SelfRoutingFlag == "" && oModelSSUDigitization.oData.PaperDataSet.Nav_Items.length < 1){
                        oModelSSUDigitization.setProperty("/BEValidation", true);
                      }
//                      oModelSSUDigitization.setProperty("/BEValidation", true);
                      oController._fnAfterGettingBEData(oData);
...
//              sap.m.MessageToast.show(vSelectedValue + " either does not exist or not extended for " + plant + " plant.");
    ...
    onSettledSub: function (oEvent) {
        var oController = this;
        var oModelSSUDigitization = oController.getView().getModel("ModelSSUDigitization");
        var selectedPath = oModelSSUDigitization.oData.SelectedPath; // AGAWSA-CONT on 04.09.2023
...
//        Calculating Apprx Cash Flow in Rs
        var vQuant = oParentModel.getProperty("/Quantity");
        var vAprxCahFlow = vQuant * vSign;
        oParentModel.setProperty("/AprxCashFlow", vAprxCahFlow.toFixed(2));
...
//        oModelSSUDigitization.setProperty("/PaperDataSet/Sign", vSign);
        ...
        if(vSign == ""){
          ...
          var nSign = vSign == "+" ? "POS" : "NEG";
        }

//        oModelSSUDigitization.setProperty(selectedPath+"/Sign", nSign);
        oModelSSUDigitization.refresh();
        oParentModel.refresh();
        oController._getAmentmendService2(nSign); //        Insert start by AGAWSA-CONT on 04.09.2023
        var oFilterData = $.grep(oModelSSUDigitization.getProperty("/F4Amentment"),function(row){
          if(vSign == "+")
            return (row.Sign == "POS");
          else if(vSign == "-")
...
//        Insert start by AGAWSA-CONT on 04.09.2023
        oModelSSUDigitization.setProperty("/F4Amentment", oFilterData);
        oModelSSUDigitization.refresh();
//      Insert end by AGAWSA-CONT on 04.09.2023
...
//      oModel.setProperty("/ExtPrice", oEvent.getSource().getSelectedItem().getAdditionalText());
      ...
      var oLastAmmndDate = new Date(vAdditionalText.split("-")[4]);
      var oOgAmndcode = vAdditionalText.split("-")[2];
      var oOgAmndcodeDesc = vAdditionalText.split("-")[3];
      oModel.setProperty("/ExtPrice", vAdditionalText.split("-")[0]);
      oModel.setProperty("/ExtValidFrom", oExtValidFrom);
      oModel.setProperty("/LastAmmndDate", oLastAmmndDate);
      oModel.setProperty("/OgAmndcode", oOgAmndcode);
      oModel.setProperty("/OgAmndcodeDesc", oOgAmndcodeDesc);
    ...
    onOgAmentmendCode: function(oEvent){
      var oModel = oEvent.getSource().getModel();
      oModel.setProperty("/OgAmndcodeDesc", oEvent.getSource().getSelectedItem().getAdditionalText());
    },
      ...
      if(ErrorData && ErrorData.hasOwnProperty("ItemNo")){
        ...
        if(ErrorData.SettledPriceFlag != ""){
          oValidationError.push({"BEError" :ErrorData.SettledPriceFlag});
                ...
                // Added by REDDRA-CONT ON 02-08-2022
        if(ErrorData.OgAmndcodeFlag != ""){
          oValidationError.push({"BEError" :ErrorData.OgAmndcodeFlag});
        }
                // Added by REDDRA-CONT ON 24-05-2022

        if(ErrorData.LastAmmndDateFlag != ""){
          oValidationError.push({"BEError" :ErrorData.LastAmmndDateFlag});
        }
...
//          oController.getView().getModel("ModelSSUDigitization").setProperty("/PaperDataSet/ACKWNLDG","X");
    ...
    _fnLockUnlockService:function(vLock) {
      ...
      var vPaperCode = oModelSSUDigitization.getProperty("/PaperDataSet/PaperCode");
      var vAssociateFlag = oModelSSUDigitization.getProperty("/PaperDataSet/AssociateFlag"); // Added by AGAWSA-CONT SF1K92101
      var vAssociate = oModelSSUDigitization.getProperty("/PaperDataSet/Associate"); // Added by AGAWSA-CONT SF1K92101
      var vInitiator = oModelSSUDigitization.getProperty("/PaperDataSet/Initiator"); // Added by AGAWSA-CONT SF1K92101
      var vCurrentUser = sap.ushell.Container.getService("UserInfo").getId(); // Added by AGAWSA-CONT SF1K92101
      ...
      $.ajax(oData_Url, {
        dataType:"json",
          success: function(oData) {
            oData = oData.d;
            busyDialog.close();
//          Add start AGAWSA-CONT SF1K92101
            if(vCurrentUser == vInitiator){
          if (vAssociateFlag == "X"){
        ...
        oController.getView().byId("id_pc").setEditable(true); // Add by AGAWSA-CONT on 15.09.2023
        }else if(vCurrentUser == vAssociate){
          if (vAssociateFlag == "X"){
            if(oData.Message.substr(0,1)=="S" && vLock != ""){
                  oModelSSUDigitization.setProperty("/EditableFlag", true);
                ...
                oController.getView().byId("id_pc").setEditable(false); // Add by AGAWSA-CONT on 15.09.2023
                }
                else if(oData.Message.substr(0,1)=="E"){
                  Message.error(oData.Message);
                  oModelSSUDigitization.setProperty("/EditableFlag", false);
