sap.ui.define(["sap/ui/core/mvc/Controller",
  ...
  var s2;
  var prevAssoName;
  return Controller.extend("com.mahindra.ZSSU_ECN.controller.ECN", {
    onInit: function () {
      // Fiori Usage 
      var device = "Other";
      var os = jQuery.os.os;
      ...
      oDataModel.read("/FioriUsageSet(Appname='ZSSU_ECN',Device='" + device +
...
//      oController._onObjectMatched(["new","ECN"]);
    },
    /*onExit: function () {
      debugger;
      console.log("aaa");
    ...
    _onObjectMatched: function (oParameter) {
      ...
      var sUrl = jQuery.sap.getModulePath('com.mahindra.ZSSU_ECN', '/json/ECN.json');
...
//      END ADD by AGAWSA-CONT 01.07.2023
    ...
    _fnAfterGettingBEData: function (oData) {
      ...
      com.mahindra.ZSSU_ECN.oData = oData;	// Added by AGAWSA-CONT 01.07.2023
...
//      Comment start by AGAWSA-CONT on 01.07.2023
//      oModelSSUDigitization.setProperty("/PaperDataSet", oData);
//      if (oData.Status === "S" || oData.Status === "R")
//        oModelSSUDigitization.setProperty("/DisplayOnlyFlag", false);
//      else if(oData.Status === "IC")
...
//      oController._getAmentmendService();
//      oController._getWorkFlow();
//    Comment end by AGAWSA-CONT on 01.07.2023
      // oController._validationDataFilled();
    },
      ...
      var oActionItem = new sap.m.HBox({
        ...
        items: [
          ...
          new sap.m.Button({
            ...
            press: function (oEvent) {
              var oItemFrag = xmlFragment(oController, "ItemECN");
...
//                contentWidth: "30%",
    ...
    _fnMassDataSet: function (vExcelDataArray) {
      ...
      $.each(vExcelDataArray, function (MainIndex, row) {
        ...
        var vSettledPrice = row.Settled_Price && row.Settled_Price.replace(/ /g, '') || "0";
        var vSanctiondBudget = row.Sanctioned_Budget && row.Sanctioned_Budget.replace(/ /g, '') || "0";
        var vQtyPerVeh = row.Qty_per_vehicle && row.Qty_per_vehicle.replace(/ /g, '') || "0";
        var vQuotedPrice = row.Quoted_Price && row.Quoted_Price.replace(/ /g, '') || "0";
        var vRemarks = row.Remarks || "";

        if (oFilter.length <= 0) {
          var oItemData = getItemDetailsObjectECN();
          oItemData.Plant = vPlant;
          oItemData.Vendor = vVendor;
          oItemData.PartNo = vPartNo;
          oItemData.SettledPrice = vSettledPrice;
          oItemData.SanctiondBudget = vSanctiondBudget;
          oItemData.QtyPerVeh = vQtyPerVeh;
          oItemData.QuotedPrice = vQuotedPrice;
          ...
          oMassData[existingIndex].SanctiondBudget = vSanctiondBudget;
          oMassData[existingIndex].QtyPerVeh = vQtyPerVeh;
          oMassData[existingIndex].QuotedPrice = vQuotedPrice;
    ...
    _validationMass: function (Nav_Items) {
      ...
      $.each(Nav_Items, function (item, Row) {
        ...
        var oNavItem = {
          ...
          "QuotedPrice": Row.QuotedPrice  // Add Start AGAWSA-CONT 30-11-2022
        };
      ...
      oController.getOwnerComponent().getModel("oDataAmendmentModel").create("/ES_HeaderX", oValidation, {
        success: function (oData, oResponse) {
//          var vMsg = "There are some issues in this price paper, please check."; START Remove BY AGAWSA-CONT ON 15-12-2022
//          var vTitle = "Information"; END Remove BY AGAWSA-CONT ON 15-12-2022
          var vIndexItem;
...
//            START Remove BY AGAWSA-CONT ON 15-12-2022
            /* 
            if (Row.VendorFlag != "" || Row.SettledPriceFlag != "" || Row.PartNoFlag != "" || Row.PlantFlag != "" || Row.ValidFromFlag != "") {
...
//            END Remove BY AGAWSA-CONT ON 15-12-2022
//            START ADD BY AGAWSA-CONT ON 15-12-2022
            if(Row.QuatedPriceFlag != ""){
...
//            END ADD BY AGAWSA-CONT ON 15-12-2022
            oController._validationDataFilled();
          });
       // START ADD by AGAWSA-CONT 15-12-2022
          if (oData.ErrorFlag == "") {
...
//          });
    ...
    _fnRequredFieldCheck: function (oFields) {
      ...
      // START ADD AGAWSA-CONT on 15-12-2022
      var today = new Date();
      ...
      var existingDate = sap.ui.getCore().byId("id_existing").mProperties.value;
      existingDate = new Date(existingDate.split(".").reverse().join("-"));
      var validFromDate = sap.ui.getCore().byId("id_validfrom").mProperties.value;
      validFromDate = new Date(validFromDate.split(".").reverse().join("-"));
      // END ADD AGAWSA-CONT on 15-12-2022
      ...
      else if(validFromDate.getTime() > today.getTime()){
    	  sap.ui.getCore().byId("id_validfrom").setValueState("Error");
          sap.ui.getCore().byId("id_validfrom").setValueStateText("Valid From Date is older than Existing Valid From date");
          oMandatoryFlag = false;
      }
...
//    	  Start Remove By AGAWSA-CONT on 15-12-2022 for warning indicator
       /* if (Row.Plant == "" || Row.Vendor == "" || Row.PartNo == "" || Math.abs(Row.SettledPrice) == "0" || Math.abs(Row.QtyPerVeh) == "0" || 
            Math.abs(Row.QuotedPrice) == "0" || Math.abs(Row.SanctiondBudget) == "0"
...
//    	  End Remove By AGAWSA-CONT on 15-12-2022
//    	  Start Add By AGAWSA-CONT on 15-12-2022 For warning indicator
    	  if (Row.Plant == "" || Row.Vendor == "" || Row.PartNo == "" || Math.abs(Row.SettledPrice) == "0" || Math.abs(Row.QtyPerVeh) == "0" || 
    	            Math.abs(Row.QuotedPrice) == "0" || Math.abs(Row.SanctiondBudget) == "0"
    	          || Row.Amndcode == "" || Row.ValidFrom == null || Row.Pinforcat == "" || Row.Purorg == "" || Row.Sign == "" ||
    	          (Row.uiFields.Error == "Reject" && Row.uiFields.MassValidation.hasOwnProperty("ItemNo"))) {
    	        ...
    	        else if(parseFloat(Row.QuotedPrice) < parseFloat(Row.SettledPrice)){  
...
//      Add start by AGAWSA-CONT 01.07.2023
        ...
        editablePaperPur: function(vEditable, vNotCreateNew, vEditableAsso){
...
//    },
    ...
    visiblePartEdit: function(vEditable, vSector, vModel,vECN, vInitiatedBySSU) {
      if (vEditable && vSector && vSector.length > 0 && vModel && vModel.length > 0 && vECN != "" && vInitiatedBySSU) {
        return true;
      } else {
        return false;
      }
    },
    visiblePartEdit1: function(vEditable, vSector, vModel,vECN, vPaperPur) {
      if (vEditable && vSector && vSector.length > 0 && vModel && vModel.length > 0 && vECN != "" && vPaperPur != "") {
        return true;
      } else {
        return false;
      }
...
//        oModelSSUDigitization.setProperty("/PaperDataSet/Initiator", "");
    ...
    onAddForexItem: function () {
      var oController = this;
      var oItemFrag = xmlFragment(oController, "ItemECN");
      var oModel = new JSONModel(getItemDetailsObjectECN());
...
//  ADD End By AGAWSA-CONT 01.07.2023
    ...
    onExport: function () {
      ...
      var oSettings = {
        workbook: {
          columns: getExportExcelColumsECN(),
