sap.ui.define(["sap/ui/core/mvc/Controller",
  ...
  "use strict";
  return Controller.extend("com.mahindra.SSU_Transportn.controller.Transportation", {
    onInit: function () {
      var device = "Other";
      var os = jQuery.os.os;
      ...
      oDataModel.read("/FioriUsageSet(Appname='ZSSU_Transportn',Device='" + device +
...
//      oController._onObjectMatched(["new","TRS"]);
    }, 
    ...
    _onObjectMatched: function (oParameter) {
      ...
      var sUrl = jQuery.sap.getModulePath('com.mahindra.SSU_Transportn', '/json/Transportation.json');
    ...
    _fnAfterGettingBEData: function (oData) {
      ...
      //added by beena-50002903 for sprint 3 
      com.mahindra.SSU_Transportn.oData = oData;
...
//                oModelSSUDigitization.setProperty("/EditableFlag", false);
      ...
      //added by beena-50002903 for sprint 3
      //commented by beena-50002903 for sprint 3
//      if (oData.Status === "S" || oData.Status === "R")
//        oModelSSUDigitization.setProperty("/DisplayOnlyFlag", false);
//      else if(oData.Status === "IC")
//        oModelSSUDigitization.setProperty("/PostToSAP", true);
...
//    added by beena-50002903 for sprint 3 on 30-06-2023
    _fnDataBeforeSave: function () {
      ...
      $.each(oPaperData.Nav_Items, function (i, Row) {
        ...
        //added by beena-50002903 for sprint 3
        if(Number(Row.ValidFrom.getFullYear().toString()) !== new Date().getFullYear()){
          Message.error("Amend can be processed only for Current Year");
          return;
        }
        //added by beena-50002903 for sprint 3
...
//                Add End by AGAWAS-CONT on 01.07.2023
              var oItemFrag = xmlFragment(oController, "ItemTransportation");
...
//
//          var oItemData = getItemDetailsObjectTRS();
...
//          oItemData.ExistngTransprt = row.Existing_Transportation || "0";
//          oItemData.SettledTransprt = row.Settled_Transportation || "0";
...
//    },
 	...
 	_fnMassDataSet: function (vExcelDataArray) {
      ...
      $.each(vExcelDataArray, function (MainIndex, row) {
        ...
        var vExistngTransprt = row.Existing_Transportation || "0";
        var vSettledTransprt = row.Settled_Transportation || "0";
        ...
        if (oFilter.length <= 0) {
          ...
          oItemData.ExistngTransprt = vExistngTransprt;
          oItemData.SettledTransprt = vSettledTransprt;
...
//          oMassData[existingIndex].Amndcode = vAmndcode;
          oMassData[existingIndex].ExistngTransprt = vExistngTransprt;
          oMassData[existingIndex].SettledTransprt = vSettledTransprt;
    ...
    _validationDataFilled: function () {
      ...
      if (oPaperData.Nav_DMS.length <= 0) {
        ...
        $.each(oPaperData.Nav_DMS, function (i, RowDMS) {
          if (RowDMS.TabId == "PCP"){
            vFlagPCP = true;
          }else if(RowDMS.TabId == "BQS"){
            vFlagBQS = true;
          }
        });
        if(vFlagPCP && vFlagBQS)
            vFlagTabId = true;
        if (!vFlagTabId) {
          vFlag = false;
          oTabIconColor.VendorPartDetails = "Negative";
...
//          oController.getView().byId("id_export").setEnabled(true);
          oModelSSUDigitization.refresh();
        },
        error: function (oResponse) {
      ...
      var oSettings = {
        workbook: {
          columns: getExportExcelColumsTRNPT(),
...
//        new sap.ui.model.Filter("Sign", sap.ui.model.FilterOperator.EQ, oPaperData.Sign),
        ...
        new sap.ui.model.Filter("AssociateFlag", sap.ui.model.FilterOperator.EQ, oPaperData.AssociateFlag), // added by beena-50002903 for sprint 3
        new sap.ui.model.Filter("Initiator", sap.ui.model.FilterOperator.EQ, oPaperData.Initiator)     // added by beena-50002903 for sprint 3
...
//        oModelSSUDigitization.setProperty("/PaperDataSet/Initiator", "");
    ...
    onAddForexItem: function () {
      var oController = this;
      var oItemFrag = xmlFragment(oController, "ItemTransportation");
      var oModel = new JSONModel(getItemDetailsObjectTRS());
...
//              if(oPaperData.Nav_Items[i].ValidFrom.split("-")[0] !== new Date().getFullYear().toString()){
//                return;
//              }
//            }
//            added by beena-50002903 for sprint 3
                ...
                if (vMsgReturn.Title == "Error") {
                  ...
                  sap.m.MessageBox.success(vMsgReturn.Message, {
                    ...
                    onClose: function () {
                      ...
                      oController.getView().byId("id_export").setVisible(true);
                    //added by beena-50002903 for sprint 3
//                      if(oModelSSUDigitization.getData().PaperDataSet.SelfRoutingFlag === "X"){
//                          oModelSSUDigitization.setProperty("/BEValidation", false); 
//                      }else{
