sap.ui.define([
  ...
  return Controller.extend("com.mahindra.SSU_AltrSource.controller.AlternateSource", {
    ...
    onInit: function () {
      ...
      oDataModel.read("/FioriUsageSet(Appname='ZSSU_AltrSource',Device='" + device +
...
//      oController._onObjectMatched(["new","AS"]);
    },
    //--------- Local Methods Start -------------------------
    _onObjectMatched: function (oParameter) {
      ...
      var sUrl = jQuery.sap.getModulePath('com.mahindra.SSU_AltrSource', '/json/AlternateSource.json');
    ...
    _fnGetNPIHeaderData: function (vPnifoNumber, vPaperCode) {
      ...
      var vParmeters = {
        "$expand": "Nav_Items/Nav_Forex,Nav_Items/Nav_RM,Nav_DMS,Nav_Log,Nav_Wf,Nav_Ret,Nav_VenSyn",
      };
...
//      oDataNewPinfoModel.read("/ES_NPIHeader('" + vPnifoNumber + "')", {
    ...
    _fnAfterGettingBEData: function (oData) {
      ...
      oData.Nav_VenSyn = oData.Nav_VenSyn.results  ? oData.Nav_VenSyn.results : oData.Nav_VenSyn;
...
//      if(oData.Initiator != oData.Createdby)
      if( oData.Initiator != "" && oData.Initiator != oData.Createdby)
...
//        delete Row.Remarks; // added temporary. needs to be removed after confirmation from odata developer
      ...
      var oActionItem = new sap.m.HBox({
        ...
        items: [
          ...
          new sap.m.Button({
            ...
            press: function (oEvent) {
              var oItemFrag = xmlFragment(oController, "ItemAlternateSource");
    ...
    _fnMassDataSet: function (vExcelDataArray) {
      ...
      $.each(vExcelDataArray, function (MainIndex, row) {
        oFilter = $.grep(oMassData, function (grepRow) {
          return ((row.Revised_Vendor_Code === grepRow.RvVendor) && (row.Plant === grepRow.Plant) && (row.Vendor_Code === grepRow.Vendor) && (row.Part_Number === grepRow.PartNo) && (row.Doc_Type === grepRow.Pinforcat));
        });
        if (oFilter.length <= 0) {
//          var vValidFrom = row.Valid_From_Date || "";

          var oItemData = getItemDetailsObjectAS();
          oItemData.BusinessRedRequired = row.Business_Red_Required == "Yes" ? "" : "X";
          ...
          oItemData.RvVendor = row.Revised_Vendor_Code || "";//
...
//          oItemData.ValidFrom = new Date(vValidFrom.split(".").reverse()) != "Invalid Date" ? new Date(vValidFrom.split(".").reverse()) : null;
          oItemData.RevisedSob = row.Revised_SOB && row.Revised_SOB.replace(/ /g, '') || "0";
...
//              Nav_Items[vIndexItem].RvVendor = Row.Vendor;
              Nav_Items[vIndexItem].ExistingSob = Row.ExistingSob;
...
//        }//Row.YoyRed == "" ||
        if (Row.BusinessRedRequired == "" && (Number(Row.Sop1yr) == 0 || Number(Row.Sop2yr) == 0
          ...
          vFlag = false;
          oTabIconColor.BusinessReduction = "Negative";
          Row.uiFields.BusiRednPanelState = "Error";
          Row.uiFields.ErrorBR = "Reject";
        } else {
          Row.uiFields.BusiRednPanelState = "Success";
          Row.uiFields.ErrorBR = "Default";
        }
      });
...
//    Add start by AGAWSA-CONT on 01.07.2023
    visibleExport: function(vEditable, vNav_Items, vNotCreateNew, vDisplayOnlyFlag){
      ...
      onExport: function () {
          ...
          var oSettings = {
            workbook: {
              columns: getExportExcelColumsAS(),
...
//      oController._fnGetTaxCodeService();
      ...
      var oModel = new JSONModel(getItemDetailsObjectAS());
    ...
    closedItemDetails: function(oEvent){
    ...
    onOkItemDetail: function (oEvent) {
      ...
      if(vChangedData.PirNo == ""){
        sap.m.MessageBox.error("P-Info Record does not exist. Create NPI Paper for "+vChangedData.Plant+" - "+vChangedData.PartNo+" - "+vChangedData.Vendor+" Combination.", {
          title: "P-Info Record does not exist"
        });
      ...
      if (vAllFilled && oFragItem.isAddNew) { // oModelSSUDigitization.getProperty("/isAddNew")
        ...
        var oFilter = $.grep(oNav_Items, function (grepRow) {
          return ((vChangedData.Pinforcat === grepRow.Pinforcat) && (vChangedData.RvVendor === grepRow.RvVendor) && (vChangedData.Plant === grepRow.Plant) && (vChangedData.Vendor === grepRow.Vendor) && (vChangedData.PartNo === grepRow.PartNo));
        });
        if (oFilter.length <= 0) {
//        	Add start by AGAWSA-CONT on 01.07.2023
        	if(vPriceUnit > 1){
        ...
        var oActionBR = new sap.m.HBox({
          ...
          items: [
            ...
            new sap.m.Button({
              icon: "{ModelDefaultDataSet>/Icon/editIcon}",
              tooltip: "Edit",
              type: "{ModelSSUDigitization>uiFields/ErrorBR}",
              visible: "{ModelSSUDigitization>/EditableFlag}",
              press: function (oEvent) {
                var oBRFrag = xmlFragment(oController, "BusinessReductionAltrS");
...
//      }
    ...
    onSynVendorValHelp: function (e) {
      var oController = this;
      var oVendor = parentXmlFragment(oController, "ValueHelp");
      oVendor.setTitle("Synthesis Vendor Search");
      var template = new sap.m.StandardListItem({
        title: "{VendName}",
        description: "{VendCode}",
        info: "{VendLoc}"
      ...
      oVendor.bindAggregation("items", "/", template);
      oVendor.setModel(new JSONModel());
      oVendor.selectedPath = e.getSource().getBindingContext("ModelSSUDigitization").getPath();
      //oVendor.parentModel = e.getSource().getBindingContext().getModel();
      oVendor.open();
    },
...
//          oParentModel.setProperty("/NetWtUnit", oSelectedData.NetWtUnit);
          ...
          oParentModel.setProperty("/ExistingSob", "0");
        ...
        if (e.getSource().getTitle() == "Ex Vendor Search") {
          ...
          oParentModel.setProperty("/ExistingSob", oSelectedData.ExistingSob);
        ...
        if (e.getSource().getTitle() == "Synthesis Vendor Search") {
          var vSelectedPath = e.getSource().selectedPath;
          oController.getView().getModel("ModelSSUDigitization").setProperty(vSelectedPath+"/VendCode", oSelectedData.VendCode);
        }
...
//          filters.push(new sap.ui.model.Filter("PartNo", sap.ui.model.FilterOperator.EQ, PartNo));
          filters.push(new sap.ui.model.Filter("VenSynFlag", sap.ui.model.FilterOperator.EQ, "X"));
        ...
        /*if(e.getSource().getTitle() == "Ex Vendor Search")
          plant = e.getSource().parentModel.getProperty("/RvPlant");*/
        ...
        oDataNewPinfoModel.read("/ES_Vendor", {
          ...
          success: function (oData, oResponse) {
            ...
            if (oData.results.length > 0 && oData.results[0].Message == "") {
              ...
              vExistingSob = oData.results[0].ExistingSob;
...
//              sap.m.MessageToast.show(vSelectedValue + " is not extended for " + oParentModel.getProperty("/Plant") + " existing plant.");
            ...
            oParentModel.setProperty("/ExistingSob", vExistingSob);
...
//        oParentModel.setProperty("/Currency", vCurrCode);
    ...
    onSubmitSysVendorCode: function (oEvent) {
      var oController = this;
      var oDataNewPinfoModel = oController.getOwnerComponent().getModel("oDataNewPinfoModel");
      var filters = [];
      var vSelectedValue = oEvent.getParameter("value").toUpperCase();
      ...
      if (vSelectedValue.length > 0) {
        ...
        oDataNewPinfoModel.read("/ES_Vendor", {
          filters: filters,
          // changes done by REDDRA-CONT ON 23-12-2021
          success: function (oData, oResponse ) {
            if (oData.results.length > 0 && oData.results[0].Message == "") {
              oModelSSUDigitization.setProperty(vSelectedPath+"/VendCode", oData.results[0].VendCode);
            } else {
//                oData.results[0].Message == "" ;
              sap.m.MessageToast.show(vSelectedValue + " does not exist.");
              oModelSSUDigitization.setProperty(vSelectedPath+"/VendCode", "");
            }
            busyDialog.close();
          },
          error: function (oResponse) {
...
//      var vButtonType = oEvent.getSource().getProperty("type");
    ...
    onAddVenSynItem: function(){
      var oController = this;
      var oModelSSUDigitization = oController.getView().getModel("ModelSSUDigitization");
      var oNav_VenSyn = oModelSSUDigitization.getProperty("/PaperDataSet/Nav_VenSyn");
      var oVenSynrData = getVendorSynObject();
      oNav_VenSyn.push(oVenSynrData);
      oModelSSUDigitization.setProperty("/PaperDataSet/Nav_VenSyn", oNav_VenSyn);
    },
    onDeleteVenSynItem: function(oEvent){
      var oController = this;
      var oModelSSUDigitization = oController.getView().getModel("ModelSSUDigitization");
