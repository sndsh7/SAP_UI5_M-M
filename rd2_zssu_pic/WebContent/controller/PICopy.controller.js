sap.ui.define([
  ...
  return Controller.extend("com.mahindra.ZSSU_PI_COPY.controller.PICopy", {
    ...
    onInit: function () {
      ...
      oDataModel.read("/FioriUsageSet(Appname='ZSSU_PI_COPY',Device='" + device +
...
//      oController._onObjectMatched(["FY21-PIC-000021","PIC"]);
    ...
    _onObjectMatched: function (oParameter) {
      ...
      var sUrl = jQuery.sap.getModulePath('com.mahindra.ZSSU_PI_COPY', '/json/PICopyData.json');
...
//      oController._uploadCollectionFilterKey();
    },
    /*_uploadCollectionFilterKey: function(){
      var oController = this;
      var oUploadCollections = oController.byId("idUploadCollectionItems").getAggregation("items");
      var oF4AttachDetails = oController.getView().getModel("ModelSSUDigitization").getProperty("/F4AttachDetails");
      $.each(oUploadCollections, function(i, Row){
        Row.getContent()[0].getBinding("items").filter([new sap.ui.model.Filter("TabId", sap.ui.model.FilterOperator.EQ, oF4AttachDetails[i].key)]);
      });
    },*/
...
//          });
    ...
    _validationDataFilled: function () {
      ...
      /*if (oPaperData.Nav_DMS.length <= 0) {
        ...
        var vFlagKey = false;
        var vF4AttachDetails = oModelSSUDigitization.getProperty("/F4AttachDetails")
        $.each(vF4AttachDetails, function (j, RowAttach) {
          if (RowAttach.mand){
            $.each(oPaperData.Nav_DMS, function (i, RowDMS) {
              if (RowDMS.TabId == RowAttach.key)
                vFlagKey = true;
            });
            if (!vFlagKey) {
              vFlag = false;
      ...
      if (oPaperData.Cycle == "" || oPaperData.Initiator == "" || oPaperData.Approver1 == "") {
        vFlag = false;
...
//  Add start by AGAWSA-CONT on 01.07.2023
    visibleExport: function(vEditable, vNav_Items, vNotCreateNew, vDisplayOnlyFlag){
      ...
      onExport: function () {
          ...
          var oSettings = {
            workbook: {
              columns: getExportExcelColumsPIC(),
...
//        oParentModel.setProperty("/Currency", vCurrCode);
    ...
    onSubmitExVendorCode: function (oEvent) {
      ...
      /*var oDataNewPinfoModel = oController.getOwnerComponent().getModel("oDataNewPinfoModel");
      var oModelSSUDigitization = oController.getView().getModel("ModelSSUDigitization");
      var filters = [new sap.ui.model.Filter("Vendor", sap.ui.model.FilterOperator.EQ, vSelectedValue)];
...
//      var vTabId = oEvent.getSource().getBindingInfo('items').binding.aFilters[0].oValue1;//for dyanamic upload collection 
