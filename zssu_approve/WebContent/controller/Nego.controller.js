sap.ui.define(["sap/ui/core/mvc/Controller",
	...
	"use strict";
	return Controller.extend("com.mahindra.ZSSU_Approve.controller.Nego", {
		onInit: function () {
			var oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			oRouter.getRoute("NEG").attachPatternMatched(this._onObjectMatched, this);
		}, 
		...
		_onObjectMatched: function (oEvent) {
			...
			var sUrl = jQuery.sap.getModulePath('com.mahindra.ZSSU_Approve', '/json/Nego.json');
...
//			oData.Sign = oData.Sign == "POS" ? 0 : 1; //0 is Increase 1 is Decrease;
			...
			$.each(oData.Nav_Items, function (i, Row) {
				...
				if (typeof Row.CurrentDate == "string"){
                    Row.CurrentDate = new Date( Row.CurrentDate);
				}
				/*Row.uiFields ={
					    "Error": "Default",
					    "ItemNo": "",
...
//			oController._getWorkFlow();
		...
		_fnDataBeforeSave: function () {
			...
			$.each(oPaperData.Nav_Items, function (i, Row) {
				...
				Row.CurrentDate = uiDateToBackend(Row.CurrentDate);
			});
			return oPaperData;
		},
			...
			var oActionItem = new sap.m.HBox({
				...
				items: [
					...
					/*new sap.m.Button({
						...
						press: function (oEvent) {
							var oItemFrag = xmlFragment(oController, "ItemNego");
...
//					oItemData.SettledPrice = row.Settled_Price || "0";
					...
					oItemData.VolDiscount = row.Volume_Discount_Per || "0";
				...
				success: function (oData, oResponse) {
					...
					$.each(oData.Nav_ItemsX, function (i, Row) {
						...
						if (Row.VendorFlag == "") {
							...
							var vExtPrice = Number(Row.ExtPrice);
							var vSettledPrice = vExtPrice - (vExtPrice * Number(Nav_Items[vIndexItem].VolDiscount) / 100);
							var vSign =  vSettledPrice - vExtPrice;
							var vChangePercnd = vSign/vExtPrice*100;
							var vVolDiscValue = Number(Nav_Items[vIndexItem].VolDiscount) * Number(Row.Quantity);
							
							vSettledPrice = (!vSettledPrice || vSettledPrice == Infinity || vSettledPrice == -Infinity) ? 0 : vSettledPrice;
							...
							Nav_Items[vIndexItem].SettledPrice = vSettledPrice.toFixed(2);
							Nav_Items[vIndexItem].VolDiscValue = vVolDiscValue.toFixed(2);
...
//			var vSelectedValue = oEvent.getParameter("value").toUpperCase();
        ...
        uploadCollectionInc: function (vSign){
        	return vSign === "POS"? true:false;
        },
        
        uploadCollectionDec: function (vSign){
        	return vSign === "NEG"? true:false;
        },
...
//			oController._getWorkFlow();
		...
		onChangeVolumDisc: function(oEvent){
			debugger;
			var oController = this;
			var oModelSSUDigitization = oController.getView().getModel("ModelSSUDigitization");
			var vNewValue = oEvent.getParameter("newValue");
			var vNewValueArray = vNewValue.split(".");
			if (vNewValueArray[0].length > 5 || (vNewValueArray[1] && vNewValueArray[1].length > 3) || vNewValue.length > 8) {
				vNewValue = vNewValueArray[1] ? vNewValueArray[0].slice(0, 5) + "." + vNewValueArray[1].slice(0, 3) : vNewValueArray[0].slice(0,
					5);
				oEvent.getSource().setValue(vNewValue);
			} else {
				oEvent.getSource().setValue(vNewValue);
			...
			var oParentModel = oEvent.getSource().getBindingContext().getModel();
			var vExtPrice = Number(oParentModel.getProperty("/ExtPrice"));
			var vQuantity = Number(oParentModel.getProperty("/Quantity"));
			
			var vSettledPrice = vExtPrice - (vExtPrice * Number(vNewValue) / 100);
			var vVolDiscValue = Number(vNewValue) * vQuantity;
			var vSign =  vSettledPrice - vExtPrice;
			var vChangePercnd = vSign/vExtPrice*100;
			...
			oParentModel.setProperty("/ChgPrctng",Math.abs(vChangePercnd).toFixed(2));
			oParentModel.setProperty("/DeltaPrice", vSign.toFixed(2));
			oParentModel.setProperty("/VolDiscValue",vVolDiscValue.toFixed(2));
			oParentModel.setProperty("/SettledPrice",vSettledPrice.toFixed(2));
			if(vSign == 0)
				vSign = "";
			else
				vSign = vSign > 0? "+" : "-";
...
//			var vNewValue = oEvent.getParameter("newValue");
			...
			oDataAmendmentModel.read("/ES_CalcCumRecQty", {
				filters: vFilters,
				success: function (oData, oResponse) {
					oParentModel.setProperty("/Quantity",oData.results[0].Quantity);
					busyDialog.close();
				},
				error: function (oResponse) {
					busyDialog.close();
