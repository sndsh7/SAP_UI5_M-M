sap.ui.define(["sap/ui/core/mvc/Controller",
  ...
  var s2;
  var prevAssoName;
  return Controller.extend("com.mahindra.SSU_Packaging.controller.Packaging", {
    onInit: function () {
      // Fiori Usage
      var device = "Other";
      var os = jQuery.os.os;
      ...
      oDataModel.read("/FioriUsageSet(Appname='ZSSU_Packaging',Device='" + device +
...
//      oController._onObjectMatched(["new","PKG"]);
    }, 
    ...
    _onObjectMatched: function (oParameter) {
      ...
      var vCurrentUser = sap.ushell.Container.getService("UserInfo").getId(); // Added by AGAWSA-CONT 01.07.2023
      var oModel = new sap.ui.model.json.JSONModel();
      var sUrl = jQuery.sap.getModulePath('com.mahindra.SSU_Packaging', '/json/Packaging.json');
      ...
      if (vPaperNo == "new") {
        ...
        oModel.setProperty("/NonSSURequest", false);
        oModel.setProperty("/SSURequest", false);	// Added by AGAWSA-CONT 01.07.2023
        var oData = getHeaderObjectPKG();
        oData.Nav_Currency.push(getCurrencyStr());
        oData.PaperCode= vPaperCode;
        oData.Createdby = vCurrentUser; // Added by AGAWSA-CONT 01.07.2023
...
//    Added by AGAWSA-CONT 01.07.2023
...
//      END ADD by AGAWSA-CONT 01.07.2023
      oController._fnCreatingTables();
    },
    visibleApprover:function(sApprover)
    {
      if(sApprover == "")
        {
        return false;
    ...
    _fnAfterGettingBEData: function (oData) {
      ...
      com.mahindra.SSU_Packaging.oData = oData;	// Added by AGAWSA-CONT SF1K92101
...
//            };
//      });
//    Add start By AGWASA-CONT on 01.07.2023
...
//    Add end By AGWASA-CONT on 01.07.2023
//      Comment insert by AGWASA-CONT on 01.07.2023
//      oModelSSUDigitization.setProperty("/NonSSURequest", false);
//      if (oData.Status === "S" || oData.Status === "R")
//        oModelSSUDigitization.setProperty("/DisplayOnlyFlag", false);
//      else if(oData.Status === "IC")
...
//      oController._getAmentmendService();
//      oController._getWorkFlow();
//    Comment end by AGWASA-CONT on 01.07.2023
      // oController._validationDataFilled();
    },
...
//                Add End by AGAWAS-CONT on 05-06-2023
              var oItemFrag = xmlFragment(oController, "ItemPackaging");
...
//          return ((row.Plant === grepRow.Plant) && (row.Vendor_Code === grepRow.Vendor) 
//              && (row.Part_Number === grepRow.PartNo) && (row.Doc_Type === grepRow.Pinforcat));
//        });
//        if (oFilter.length <= 0) {
//          var vValidFrom = row.Valid_From_Date || "";
//
//          var oItemData = getItemDetailsObjectPKG();
...
//          oItemData.ExistngPackg = row.Existing_Packaging || "";
//          oItemData.SettldPackg = row.Settled_Packaging || "";
...
//    },
 	_fnMassDataSet: function (vExcelDataArray) {
      ...
      $.each(vExcelDataArray, function (MainIndex, row) {
        ...
        var vExistngPackg = row.Existing_Packaging || "0";
        var vSettldPackg = row.Settled_Packaging || "0";
        ...
        if (oFilter.length <= 0) {
          ...
          oItemData.ExistngPackg = vExistngPackg;
          oItemData.SettldPackg = vSettldPackg;
          ...
          oMassData[existingIndex].ExistngPackg = vExistngPackg;
          oMassData[existingIndex].SettldPackg = vSettldPackg;
...
//        }
      ...
      var oSettings = {
        workbook: {
          columns: getExportExcelColumsPCKNG(),
...
//          var vTitle = "Information";
          var vIndexItem;
//          *************************copied from PA************************\
//          oData.InitiName = "Varun Mohan";
//          oData.Appr1Name = "Saurabh Kadam";
//          oData.Approver1 = "BCONE-60314";
//          oData.Initiator = "MOHAVA-CONT";
//          oData.Cycle = "01";
          /*if(oData.Cycle != ""){
...
//          busyDialog.close();
    ...
    _fnRequredFieldCheck: function (oFields) {
        ...
        var validFromDate = oFields[44]._lastValue;
      ...
      oFields[44].setValueState("Error");
      oFields[44].setValueStateText("Valid From Date is older than Existing Valid From date");
      oMandatoryFlag = false;
        }
...
//      if (vEditable && vSector && vSector.length > 0 && vModel && vInitiatedBySSU) {
//        return true;
//      } else {
//        return false;
//      }
...
//        oModelSSUDigitization.setProperty("/PaperDataSet/Initiator", "");
    ...
    onAddForexItem: function () {
      var oController = this;
      var oItemFrag = xmlFragment(oController, "ItemPackaging");
      var oModel = new JSONModel(getItemDetailsObjectPKG());
    ...
    onOkItemDetail: function (oEvent) {
      ...
      // perform existing and settled price validation
      var vItemSign = oEvent.getSource().getParent().getModel().getProperty("/Sign");
      if(vItemSign == "") {
        sap.m.MessageToast.show("Settled Price should be either great than or less than Existing Price.");
        return;
      }
...
//    Added End by AGAWSA-CONT 01.07.2023
...
//              sap.m.MessageToast.show(vSelectedValue + " either does not exist or not extended for " + plant + " plant.");
    ...
    onSettledSub: function (oEvent) {
        ...
        oModel.setProperty("/AmndcodeDesc", oEvent.getSource().getValue());
        var oParentModel = oEvent.getSource().getBindingContext().getModel();
...
//      var slug =vPaperCode+"|"+vPaperNo+"|CWB|0|0|0|0|"+oFile['name']+"|"+oFile['type']+"|"+oFile['size']+"";
...
//      oController._getWorkFlow();
    ...
    onChangePaperPur: function(oEvent){
      ...
      oController._getAmentmendService();
      var oModelSSUDigitization = oController.getView().getModel("ModelSSUDigitization");
      oModelSSUDigitization.setProperty("/PaperDataSet/Disclaimer",oEvent.getSource().getSelectedItem().getAdditionalText());
...
//  ADD End By AGAWSA-CONT 01.07.2023
    _fnLockUnlockService:function(vLock) {
      ...
      var vAssociateFlag = oModelSSUDigitization.getProperty("/PaperDataSet/AssociateFlag"); // Added by AGAWSA-CONT 01.07.2023
      var vAssociate = oModelSSUDigitization.getProperty("/PaperDataSet/Associate"); // Added by AGAWSA-CONT 01.07.2023
      var vInitiator = oModelSSUDigitization.getProperty("/PaperDataSet/Initiator"); // Added by AGAWSA-CONT 01.07.2023
      var vCurrentUser = sap.ushell.Container.getService("UserInfo").getId(); // Added by AGAWSA-CONT 01.07.2023
      ...
      $.ajax(oData_Url, {
        dataType:"json",
          success: function(oData) {
            oData = oData.d;
            busyDialog.close();
//          Add start AGAWSA-CONT 01.07.2023
            if(vCurrentUser == vInitiator){
          if (vAssociateFlag == "X"){
...
//          Add end AGAWSA-CONT 01.07.2023
//          if(oData.Message.substr(0,1)=="S" && vLock != ""){
//            oModelSSUDigitization.setProperty("/EditableFlag", true);
//            oModelSSUDigitization.setProperty("/BEValidation", false);
//          }
