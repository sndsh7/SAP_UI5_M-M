sap.ui.define(["sap/ui/core/mvc/Controller",
  ...
  var s2;
  var prevAssoName;
  return Controller.extend("com.mahindra.ZSSU_Rocm.controller.Rocm", {
    onInit: function () {
      // Fiori Usage
      var device = "Other";
      var os = jQuery.os.os;
      ...
      oDataModel.read("/FioriUsageSet(Appname='ZSSU_Rocm',Device='" + device +
...
//      oController._onObjectMatched(["new","RCM"]);
    }, 
    ...
    _onObjectMatched: function (oParameter) {
      ...
      var sUrl = jQuery.sap.getModulePath('com.mahindra.ZSSU_Rocm', '/json/Rocm.json');
...
//      END ADD by AGAWSA-CONT 01.07.2023
    ...
    _fnAfterGettingBEData: function (oData) {
      ...
      com.mahindra.ZSSU_Rocm.oData = oData;	// Added by AGAWSA-CONT 01.07.2023
      ...
      var oActionItem = new sap.m.HBox({
        ...
        items: [
          ...
          new sap.m.Button({
            ...
            press: function (oEvent) {
              var oItemFrag = xmlFragment(oController, "ItemRoCM");
    ...
    _fnMassDataSet: function (vExcelDataArray) {
      ...
      $.each(vExcelDataArray, function (MainIndex, row) {
        ...
        if (oFilter.length <= 0) {
          ...
          var oItemData = getItemDetailsObjectRCM();
    ...
    _validationDataFilled: function () {
      ...
      if (oPaperData.Nav_DMS.length <= 0) {
        ...
        $.each(oPaperData.Nav_DMS, function (i, RowDMS) {
          if (RowDMS.TabId == "SD"){
            vFlagSD = true;
          }else if(RowDMS.TabId == "BU"){
            vFlagBU = true;
          }else if(RowDMS.TabId == "VFM"){
            vFlagVFM = true;
          }else if(RowDMS.TabId == "SBC"){
            vFlagSBC = true;
          }else if(RowDMS.TabId == "FTB"){
            vFlagFTB = true;
          }
        });
        if(vFlagSD && vFlagBU && vFlagVFM && vFlagSBC && vFlagFTB)
            vFlagTabId = true;
        if (!vFlagTabId) {
          vFlag = false;
          oTabIconColor.VendorPartDetails = "Negative";
      ...
      $.each(oPaperData.Nav_Items, function (i, Row) {
        ...
        if (Row.Plant == "" || Row.Vendor == "" || Row.PartNo == "" ||  Math.abs(Row.SettledPrice) == "0" || Math.abs(Row.QtyPerVeh) == "0" || 
            Math.abs(Row.LOIPrice) == "0" || Math.abs(Row.SanctiondBudget) == "0"
        ...
        oDataAmendmentModel.read("/ES_Vendor", {
          ...
          success: function (oData, oResponse) {
            ...
            if(vPinforcat == oData.results[0].Pinforcat || vPinforcat == ""){
              ...
              /*if(oParentModel.getProperty("/Pinforcat") != "" ){
                vPinforcat = oParentModel.getProperty("/Pinforcat");
                vPinfoCatText =oParentModel.getProperty("/PinfoCatText");
              }*/
              }
              else{
...
//        Add end by AGAWSA-CONT 01.07.2023
        formatterLoiEdit: function(vEditable,vLOIPrice){
      return vEditable && (vLOIPrice > 0 ? false :true );
    },
...
//        oModelSSUDigitization.setProperty("/PaperDataSet/Initiator", "");
    ...
    onAddForexItem: function () {
      var oController = this;
      var oItemFrag = xmlFragment(oController, "ItemRoCM");
      var oModel = new JSONModel(getItemDetailsObjectRCM());
...
//      oController.getView().getModel("ModelSSUDigitization").setProperty("/BEValidation", false);
    ...
    onSave: function () {
      ...
      var oModelSSUDigitization = oController.getView().getModel("ModelSSUDigitization");
//    Added by AGAWSA-CONT 01.07.2023
      if (oController.getView().byId('id_model').getValue() == ""
...
//      var vSelectedIndex;
    ...
    onExport: function () {
      ...
      var oSettings = {
        workbook: {
          columns: getExportExcelColumsROCM(),
...
//              sap.m.MessageToast.show(vSelectedValue + " either does not exist or not extended for " + plant + " plant.");
    ...
    onSelectionChangeItemAtta: function(oEvent){
      ...
      var oParentModel = oEvent.getSource().getBindingContext().getModel();
      var filters = [
        new sap.ui.model.Filter("PaperCode", sap.ui.model.FilterOperator.EQ, oModelSSUDigitization.getProperty("/PaperDataSet/PaperCode")),
        new sap.ui.model.Filter("Plant", sap.ui.model.FilterOperator.EQ, oParentModel.getProperty("/Plant")),
        new sap.ui.model.Filter("VendCode", sap.ui.model.FilterOperator.EQ, oParentModel.getProperty("/Vendor")),
        new sap.ui.model.Filter("PartNo", sap.ui.model.FilterOperator.EQ, oParentModel.getProperty("/PartNo")),
        new sap.ui.model.Filter("Pinforcat", sap.ui.model.FilterOperator.EQ, oParentModel.getProperty("/Pinforcat"))
        ];
      oDataAmendmentModel.read("/ES_LOIPrice", {
        filters: filters,
        success: function (oData, oResponse) {
          oParentModel.setProperty("/LOIPrice",oData.results[0].LOIPrice);
        },
        error: function (oResponse) {
          Message.error("Error loading Initiator.", {
            title: "Error"
