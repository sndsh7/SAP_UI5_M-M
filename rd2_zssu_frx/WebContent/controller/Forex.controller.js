sap.ui.define(["sap/ui/core/mvc/Controller",
  ...
  "use strict";
  return Controller.extend("com.mahindra.ZSSU_Forex.controller.Forex", {
    onInit: function () {
      // Fiori Usage
      var device = "Other";
      var os = jQuery.os.os;
      ...
      oDataModel.read("/FioriUsageSet(Appname='ZSSU_FOREX',Device='" + device +
...
//      jQuery.sap.registerModulePath("SSU", "http://mcvladsf1.corp.mahindra.com:8000/sap/bc/ui5_ui5/sap/zssu");
       var vStartupPara = getStartupParameters(this);
       if(vStartupPara.PaperNumber.length >= 2)
    ...
    _onObjectMatched: function (oParameter) {
      ...
      var sUrl = jQuery.sap.getModulePath('com.mahindra.ZSSU_Forex', '/json/forex.json');
      oModel.loadData(sUrl, "", false);
      oController.getView().setModel(oModel, "ModelSSUDigitization");
      oController.getView().byId("redelegation").setVisible(false) // added by beena on 14-03-2023ś

      oModel.setProperty("/PartSettlementTracker", oParameter[2] || "");
//      oModel.setProperty("/Title",this.getView().getParent().vTitle);
...
//      END ADD by AGAWSA-CONT 
    ...
    _fnAfterGettingBEData: function (oData) {
      ...
      com.mahindra.ZSSU_Forex.oData = oData //added by beena on 14-03-2023
...
//      oData.Sign = oData.Sign == "POS" ? 0 : 1; //0 is Increase 1 is Decrease;
      $.each(oData.Nav_Currency, function (i, Row) {
        if (typeof Row.ExFrmDt == "string"){
                    Row.ExFrmDt = new Date( Row.ExFrmDt);
        }
        if (typeof Row.ExToDt == "string"){
                    Row.ExToDt = new Date( Row.ExToDt);
        }
        if (typeof Row.RvFrmDt == "string"){
                    Row.RvFrmDt = new Date( Row.RvFrmDt);
        }
        if (typeof Row.RvToDt == "string"){
                    Row.RvToDt = new Date( Row.RvToDt);
        }
        oData.ExFrmDt = Row.ExFrmDt;
        oData.ExToDt = Row.ExToDt;
        oData.RvFrmDt = Row.RvFrmDt;
        oData.RvToDt = Row.RvToDt;
      });
...
//      Remove Start by AGAWSA-CONT on 02-02-23
//      if (oData.Status === "S" || oData.Status === "R")
//        oModelSSUDigitization.setProperty("/DisplayOnlyFlag", false);
//      else if(oData.Status === "IC")
//        oModelSSUDigitization.setProperty("/PostToSAP", true);
//    Remove End by AGAWSA-CONT on 02-02-23
...
//      oPaperData.Sign = oPaperData.Sign == 0 ? "POS" : "NEG",
      ...
      oPaperData.Nav_Ret = [];
      oPaperData.Nav_DMS = [];
      $.each(oPaperData.Nav_Currency, function (i, Row) {
        Row.ExFrmDt = uiDateToBackend(oPaperData.ExFrmDt);
        Row.ExToDt = uiDateToBackend(oPaperData.ExToDt);
        Row.RvFrmDt = uiDateToBackend(oPaperData.RvFrmDt);
        Row.RvToDt = uiDateToBackend(oPaperData.RvToDt);
        delete Row.__metadata;
      });
      delete oPaperData.ExFrmDt;
      delete oPaperData.ExToDt;
      delete oPaperData.RvFrmDt;
      delete oPaperData.RvToDt;
      $.each(oPaperData.Nav_Items, function (i, Row) {
        delete Row.DeltaPrice; // need to remove once add DeltaPrice in table level
...
//            oItemFrag.getAggregation("buttons")[0].setVisible(oParentModel.getProperty("/uiFields/MassValidation/ItemNo")?true:false);
              oItemFrag.open();
...
//                contentWidth: "30%",
    ...
    _fnDataSettingValidation: function (vSelectedPathVali) {// need to delete
    ...
    _fnMassDataSet: function (vExcelDataArray) {
      ...
      $.each(vExcelDataArray, function (MainIndex, row) {
        ...
        if (oFilter.length <= 0) {
          var oItemData = getItemDetailsObjectFRX();
...
//      Commented by AGAWSA-CONT-SF1K920445
...
//    Remove END by AGAWSA-CONT on 23-01-2023
      $.each(oPaperData.Nav_Items, function (i, Row) {
        ...
        if (Row.Plant == "" || Row.Vendor == "" || Row.PartNo == "" || Row.SettledPrice == "" || Row.SettledPrice == "0" || Row.Sign == ""
          || Row.Amndcode == "" || Row.ValidFrom == null || Row.Pinforcat == "" || Row.Purorg == "" || (Row.uiFields.Error == "Reject" && Row.uiFields.MassValidation.hasOwnProperty("ItemNo")) ) {
          vFlag = false;
          oTabIconColor.VendorPartDetails = "Negative";
    ...
    _getXsrfToken: function(oEvent) {
            ...
            if (!token) {
                this.getOwnerComponent().getModel("oDataAmendmentModel").refreshSecurityToken(function(e, o) {
                    ...
                    Message.error("Error while getting x-csrf-token", {
            title: "Error"
          });
                }, false);
             }
...
//      Add End by AGAWAS-CONT on 28-01-2023
      var oItemFrag = xmlFragment(oController, "ItemForex");
      var oModel = new JSONModel(getItemDetailsObjectFRX());
    ...
    onSave: function () {
      var oController = this;
      var oModelSSUDigitization = oController.getView().getModel("ModelSSUDigitization");
//    Added start by AGAWSA-CONT-SF1K920445
...
//    Commented by AGAWSA-CONT-SF1K920445
////    Added Start by AGAWSA-CONT on 28-01-2023
...
//        }
//      }
////    Added End by AGAWSA-CONT on 28-01-2023
//    Added by AGAWSA-CONT-SF1K920445
          ...
          if (oAction == "OK") {
            ...
            oController.getOwnerComponent().getModel(
              ...
              success: function (oData, oResponse) {
                var vMsgReturn = serviceSucess(oData.Nav_Ret.results);
                if (vMsgReturn.Title == "Error") {
                  Message.error(vMsgReturn.Message, {
                    title: vMsgReturn.Title
                  });
                  oController._fnAfterGettingBEData(oPaperData);
                } else if (vMsgReturn.Title == "Sucess") {
                  Message.success(vMsgReturn.Message, {
                    title: vMsgReturn.Title,
                    onClose: function () {
...
//          }
//      }
////    Add End by AGAWSA-CONT on 27-01-2023
//    Added by AGAWSA-CONT-SF1K920445
      if(vCurrentUser == vInitiator){
        if(oModelSSUDigitization.oData.PaperDataSet.Nav_Items.length <= 0 && oModelSSUDigitization.oData.PaperDataSet.AssociateFlag == "" 
...
//              sap.m.MessageToast.show(vSelectedValue + " either does not exist or not extended for " + plant + " plant.");
    ...
    onChangePaperPur: function(oEvent){
      var oController = this;
      /*var oModelSSUDigitization = oController.getView().getModel("ModelSSUDigitization");
      if(oEvent.getSource().getSelectedKey() == "03")
...
//  ADD Start By AGAWSA-CONT on 20-01-2023
...
//          Remove End by AGAWSA-CONT on 25-01-2023
    ...
    onExport: function () {
      ...
      var oSettings = {
        workbook: {
          columns: getExportExcelColumsFRX(),
