sap.ui.define(["sap/ui/core/mvc/Controller",
	...
	"use strict";
	return Controller.extend("com.mahindra.ZSSU_Approve.controller.PriceActualisation", {
		onInit: function () {
			// Fiori Usage
			var oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			oRouter.getRoute("PA").attachPatternMatched(this._onObjectMatched, this);
		}, 
		...
		_onObjectMatched: function (oEvent) {
			...
			var sUrl = jQuery.sap.getModulePath('com.mahindra.ZSSU_Approve', '/json/PriceActlsn.json');
			...
			oController.getOwnerComponent().getModel("oDataAmendmentModel").create("/ES_HeaderX", oValidation, {
				success: function (oData, oResponse) {
					...
					$.each(oData.Nav_ItemsX, function (i, Row) {
						...
						if (Row.VendorFlag != "" || Row.PartNoFlag != "" || Row.AmndcodeFlag != "" || Row.PlantFlag != "" || Row.ValidFromFlag != "") {
		...
		_validationDataFilled: function () {
			...
			if (oPaperData.Nav_DMS.length <= 0) {
				...
				$.each(oPaperData.Nav_DMS, function (i, RowDMS) {
					if (RowDMS.TabId == "MAC" || RowDMS.TabId == "CT" || RowDMS.TabId == "WT" || RowDMS.TabId == "DMS")
						vFlagTabId = true;
				});
				if (!vFlagTabId) {
					vFlag = false;
...
//		Quantity service on Change Vaid from date
		onChangeValidFrom : function(oEvent){
			...
			vFilters.push(new sap.ui.model.Filter("ValidFrom", sap.ui.model.FilterOperator.EQ, oController._uiDateToBE(oParentModel.getProperty("/ValidFrom"))));
			busyDialog.open();
					...
					onClose: function (oAction) {
						if (oAction == "OK") {
							...
							if(oSelectedItems.length == 0){
								sap.m.MessageBox.error("Select items or All selected items are already processed");
								oController._fnAfterGettingBEData(oPaperData);
								return;
							}
