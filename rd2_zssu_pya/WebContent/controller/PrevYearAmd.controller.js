sap.ui.define(["sap/ui/core/mvc/Controller",
  ...
  var s2;
  var prevAssoName;
  return Controller.extend("com.mahindra.ZSSU_PrvYrAmndt.controller.PrevYearAmd", {
    onInit: function () {
      // Fiori Usage
      var device = "Other";
      var os = jQuery.os.os;
      ...
      oDataModel.read("/FioriUsageSet(Appname='ZSSU_PrvYrAmndt',Device='" + device +
...
//      oController._onObjectMatched(["new","PC"]);
    ...
    _onObjectMatched: function (oParameter) {
      ...
      var sUrl = jQuery.sap.getModulePath('com.mahindra.ZSSU_PrvYrAmndt', '/json/PrvYrAmndt.json');
...
//      END ADD by AGAWSA-CONT
    ...
    _fnAfterGettingBEData: function (oData) {
      ...
      com.mahindra.ZSSU_PrvYrAmndt.oData = oData;	// Added by AGAWSA-CONT SF1K92101
...
//      oData.Sign = oData.Sign == "POS" ? 0 : 1; //0 is Increase 1 is Decrease;
      ...
      $.each(oData.Nav_Items, function (i, Row) {
        var vFYdate = minDatePYA();
        vFYdate.setDate(vFYdate.getDate() + 1);
        Row.uiFields ={
              "Error": "Default",
              "ItemNo": "",
              "maxDate":  new Date(),
...
//      oController.getView().byId("id_export").setVisible(true);
      ...
      //added by beena-50002903 on 19-06-2023
//      if(oData.PYIFlag === "X"){
//        oModelSSUDigitization.setProperty("/EditableFlag", false);
//      }else{
...
//      if (oModelSSUDigitization.getProperty("/PartSettlementTracker") == "X")
//          oModelSSUDigitization.setProperty("/DisplayOnlyFlag", true);
//      else if (oModelSSUDigitization.getProperty("/PartSettlementTracker") == "Y")
//          oModelSSUDigitization.setProperty("/DisplayOnlyFlag", true);
//      else if (oData.Status === "S" || oData.Status === "R")
//        oModelSSUDigitization.setProperty("/DisplayOnlyFlag", false);
//      else if(oData.Status === "IC")
//        oModelSSUDigitization.setProperty("/PostToSAP", true);
//      oModelSSUDigitization.setProperty("/PaperDataSet", oData);
...
//                Add End by AGAWAS-CONT on 05-06-2023
              var oItemFrag = xmlFragment(oController, "ItemPrvYrAmndt");
...
//                contentWidth: "30%",
    ...
    _setEditFragmentModel: function (oEvent) {
      ...
      //added by beena-50002903 on 17-06-2023  as hardcode FY year first Date as 01-04-2023
//      var paperNo = objectData.PaperNo.split("-")[0];
//      if(paperNo === "FY24" || paperNo === ""){
//        objectData.ValidFrom = new Date('04.01.2023')
//      }
//      oController.getView().getModel("ModelSSUDigitization").refresh()
    //added by beena-50002903 on 17-06-2023
    ...
    _setDisplayFragmentModel: function (oEvent) {
      ...
      var vSelectedPath = oEvent.getSource().getBindingContext("ModelSSUDigitization").getPath();
      //added by beena-50002903 on 17-06-2023  as hardcode FY year first Date as 01-04-2023
      var  selectObject = oController.getView().getModel("ModelSSUDigitization").getProperty(vSelectedPath);
      var paperNo = selectObject.PaperNo.split("-")[0];
      if(paperNo === "FY24"){
        selectObject.ValidFrom = new Date('04.01.2023')
      }
      oController.getView().getModel("ModelSSUDigitization").refresh()
    //added by beena-50002903 on 17-06-2023
...
//
//          var oItemData = getItemDetailsObjectPYA();
...
//uncomment by beena on 26-02-2024
            ...
            //commendted by beena on 26-02-2023
//            if(Row.AmndcodeFlag == ""){
//                Nav_Items[vIndexItem].Amndcode = Row.Amndcode;
//              Nav_Items[vIndexItem].AmndcodeDesc = Row.AmndcodeDesc;
//            }
            if (Row.VendorFlag != "" || Row.PartNoFlag != "" || Row.SettledPriceFlag !== "" || Row.AmndcodeFlag != "" || Row.PlantFlag != "" || Row.ValidFromFlag != ""
              || Row.Messg != "") {
...
//          oModelSSUDigitization.setProperty("/Validation", oData);
//          
//          //added by beena-50002903 on 20-06-2023
//          Nav_Items.sort(function(a,b){
//            if(a.uiFields.Error < b.uiFields.Error){
//              return -1;
//            }
...
//          busyDialog.close();
    ...
    _validationDataFilled: function () {
      ...
      if (oPaperData.Nav_DMS.length <= 0) {
        ...
        var vFlagPAC = true; // make true by beena-50002903 on 17-06-2023
        var vFlagPB = false;
        var vFlagAB = false;
        $.each(oPaperData.Nav_DMS, function (i, RowDMS) {
          if (RowDMS.TabId == "PAC"){
            vFlagPAC = true;
          }else if(RowDMS.TabId == "PB"){
            vFlagPB = true;
          }else if(RowDMS.TabId == "AB"){
            vFlagAB = true;
          }
        });
        if(vFlagPAC && vFlagPB && vFlagAB)
            vFlagTabId = true;
        if (!vFlagTabId) {
          vFlag = false;
          oTabIconColor.VendorPartDetails = "Negative";
...
//          ADD End by AGAWSA-CONT on 14-02-2023 validation error message for inactive users
      ...
      var oSettings = {
        workbook: {
          columns: getExportExcelColumsPYAMD(),
...
//        Add End by AGAWSA-CONT SF1K92101
    ...
    formattSignF4Amnd: function(vSign){
      if(vSign){
        ...
        return vSign+" VE";    //uncomment by beena on 26-02-2024
//        formatterSetSign(vSign); // comment by beena on 26-02-2024
      }
...
//        oModelSSUDigitization.setProperty("/PaperDataSet/Initiator", "");
    ...
    onAddForexItem: function () {
      var oController = this;
      var oItemFrag = xmlFragment(oController, "ItemPrvYrAmndt");
      var oModel = new JSONModel(getItemDetailsObjectPYA());
      //added by beena-50002903 on 19-06-2023
      oModel.getData().ValidFrom = new Date('04.01.2023')
//      added by beena-50002903 on 19-06-2023
      oItemFrag.setModel(oModel);
      oItemFrag.getModel().refresh() // added by beena-50002903 on 19-06-2023
...
//    Added by AGAWSA-CONT SF1K92101
      if ((oController.getView().byId('id_model').getValue() == "" && oController.getView().byId('id_sector').getValue() == "") ||
          (oController.getView().byId('id_PYI').mProperties.selectedIndex == 0)){
        if(oController.getView().byId('id_pnn')._lastValue == ""){
//          oController.getView().byId('id_pnn').setValueState("Error");
...
//  ADD Start By AGAWSA-CONT SF1K92101
    ...
    //added by beena-50002903 on 17-06-2023
    onSelectPYI : function(evt){
      debugger
      var oController = this;
        var oModelSSUDigitization = oController.getView().getModel("ModelSSUDigitization");
        var vSelect =evt.getParameter("selectedIndex") == 0 ? "X" : ""; //0 is Yes and 1 is No;
        oModelSSUDigitization.setProperty("/PaperDataSet/PYIFlag",vSelect);
    },
    //added by beena-50002903 on 17-06-2023
