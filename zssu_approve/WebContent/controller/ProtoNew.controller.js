sap.ui.define(["sap/ui/core/mvc/Controller",
  ...
  "use strict";
  var vNav_Apprs = [];
  return Controller.extend("com.mahindra.ZSSU_Approve.controller.ProtoNew", {
...
//      oData.Model = oData.Model ? oData.Model.split(",") : [];
      ...
      oData.Nav_Items = oData.Nav_Itms.results ? oData.Nav_Itms.results : oData.Nav_Items;
...
//      var val = selectedPath.split("/")[3];
    ...
    onApprove: function (oEvent) {
      ...
      var oPaperData = oController._fnDataBeforeSave();
      var oDataSet={};
      var oSelectedPath = oController.getView().byId("idProtoTBL").getSelectedContextPaths();
      if(oSelectedPath.length <= 0){
        return MessageBox.error("Please select items to approve");
      }
      var oNav_Items=[];
      $.each(oSelectedPath, function(i,Row){
        oNav_Items.push({
              "Posnr": oModelSSUDigitization.getProperty(Row).Posnr,
              "Nav_Ret":[]
            });
      });
       oDataSet = {
    ...
    onWorkflowDisplay:function(oEvent){
      debugger;
      var oController = this;
      var oWorkFlowFrag = xmlFragment(oController, "workFlow");
      oWorkFlowFrag.setModel(oController._setDisplayFragmentModel(oEvent));
      oWorkFlowFrag.open();
    }
